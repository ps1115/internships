...
dbSPE.define_table('usuario_profesor',
                Field('usbid_usuario','string', requires=[IS_NOT_EMPTY(), IS_MATCH('[a-zA-Z][a-zA-Z0-9_]*@usb.ve','USBID Inválido'),IS_NOT_IN_DB(dbSPE, 'usuario_profesor.usbid_usuario',error_message=T('Usuario ya existe'))], label='USBID', ondelete='CASCADE', notnull=True,unique=True),
